...
public enum VehicleHud {
    ...
    public static void render(MatrixStack matrices, PlayerEntity player, VehicleEntity auto, float tickDelta) {
    ...
    private static void renderSpeedometer(MatrixStack matrices, VehicleEntity auto) {
        float speed = (float) auto.getEffectiveSpeed() * 20;
        int color = 0xFFFFFF;
        if (auto.getBoostTimer() > 0) color = 0xFF6F00;
        if (auto.getTurboCharge() > VehicleEntity.SMALL_TURBO_TIME) color = 0xFFEA4A;
        if (auto.getTurboCharge() > VehicleEntity.MEDIUM_TURBO_TIME) color = 0x7DE9FF;
        if (auto.getTurboCharge() > VehicleEntity.LARGE_TURBO_TIME) color = 0x906EFF;
