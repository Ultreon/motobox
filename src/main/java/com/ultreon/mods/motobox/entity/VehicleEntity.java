...
import com.ultreon.mods.motobox.item.VehicleInteractable;
...
public class VehicleEntity extends Entity implements RenderableVehicle, EntityWithInventory {
        ...
        for (var entity : world.getEntitiesByType(TypeFilter.instanceOf(Entity.class), frontBox, entity -> entity != this && entity != getFirstPassenger())) {
            if (!entity.isInvulnerable()) {
                if (entity instanceof LivingEntity living && entity.getVehicle() != this) {
                    living.damage(MotoboxEntities.AUTOMOBILE_DAMAGE_SOURCE, hSpeed * 10);
    ...
    private void dropParts(Vec3d pos) {
        world.spawnEntity(new ItemEntity(world, pos.x, pos.y, pos.z, MotoboxItems.AUTOMOBILE_FRAME.createStack(this.getFrame())));
        world.spawnEntity(new ItemEntity(world, pos.x, pos.y, pos.z, MotoboxItems.AUTOMOBILE_ENGINE.createStack(this.getEngine())));
