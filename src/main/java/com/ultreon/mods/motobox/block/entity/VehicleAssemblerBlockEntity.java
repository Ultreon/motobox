package com.ultreon.mods.motobox.block.entity;

import com.ultreon.mods.motobox.block.MotoboxBlocks;
import com.ultreon.mods.motobox.block.VehicleAssemblerBlock;
import com.ultreon.mods.motobox.entity.VehicleEntity;
import com.ultreon.mods.motobox.item.MotoboxItems;
import com.ultreon.mods.motobox.item.VehicleEngineItem;
import com.ultreon.mods.motobox.item.VehicleFrameItem;
import com.ultreon.mods.motobox.item.VehicleWheelItem;
import com.ultreon.mods.motobox.vehicle.VehicleEngine;
import com.ultreon.mods.motobox.vehicle.VehicleFrame;
import com.ultreon.mods.motobox.vehicle.VehicleStats;
...
public class VehicleAssemblerBlockEntity extends BlockEntity implements RenderableVehicle {
    ...
    public VehicleAssemblerBlockEntity(BlockPos pos, BlockState state) {
        super(MotoboxBlocks.AUTOMOBILE_ASSEMBLER_ENTITY, pos, state);
    }
    ...
    protected ActionResult handleItemInteract(PlayerEntity player, ItemStack stack) {
        ...
        if (!this.world.isClient() && stack.isOf(MotoboxItems.FRONT_ATTACHMENT) || stack.isOf(MotoboxItems.REAR_ATTACHMENT)) {
            player.sendMessage(VehicleAssemblerBlock.INCOMPLETE_AUTOMOBILE_DIALOG, true);
        }
    ...
    public void tryConstructVehicle() {
        if (this.isComplete()) {
            ...
            var auto = new VehicleEntity(this.world);
            auto.refreshPositionAndAngles(pos.x, pos.y, pos.z, this.getVehicleYaw(0), 0);
    ...
    public void dropParts() {
        ...
        this.world.spawnEntity(new ItemEntity(world, pos.x, pos.y, pos.z, MotoboxItems.AUTOMOBILE_FRAME.createStack(this.getFrame())));
        this.world.spawnEntity(new ItemEntity(world, pos.x, pos.y, pos.z, MotoboxItems.AUTOMOBILE_ENGINE.createStack(this.getEngine())));

        var wheelStack = MotoboxItems.AUTOMOBILE_WHEEL.createStack(this.getWheels());
